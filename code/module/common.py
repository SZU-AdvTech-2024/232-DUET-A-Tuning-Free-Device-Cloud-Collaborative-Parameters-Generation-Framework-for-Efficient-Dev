...
class HyperNetwork_FC(nn.Module):
    ...
    def __init__(self,
        ...
        self.modules = []
        # print("id_dimension", model_conf.id_dimension)
        units = [in_dimension] + list(units)
        self.activation_fns = activation_fns
        self.batch = batch
        self.units = units
        self.w1 = []  #w1、b1用来生成weight
        self.w2 = []  #w1、b1用来生成weight
        self.b1 = []
        self.b2 = []
        self.output_size = []
        self._gru_cell = torch.nn.GRU(model_conf.id_dimension,
        ...
        self.expand = expand
        #units:[in_dimension,id_dimension,id_dimension]
        for i in range(1, len(units)):
            ...
            self.w1.append(
                Parameter(
                    torch.fmod(
                        torch.randn(in_dimension,
                                    input_size * output_size).cuda(), 2)))
            self.b1.append(
                Parameter(
                    torch.fmod(
                        torch.randn(input_size * output_size).cuda(), 2)))

            self.w2.append(
                Parameter(
                    torch.fmod(
                        torch.randn(in_dimension, output_size).cuda(), 2)))
            self.b2.append(
                Parameter(torch.fmod(torch.randn(output_size).cuda(), 2)))
    ...
    def forward(self, x, z, sample_num=32, trigger_seq_length=30):
        ...
        # print("z1.size", z.size())
        user_state = user_state[range(user_state.shape[0]),
                                trigger_seq_length, :]
        # print("user_state.size", user_state.size())
        z = self._mlp_trans(user_state)
        # print("x.size", x.size())
        # print("z2.size", z.size())
        ...
        for i in range(1, len(units)):
            ...
            if i == 1 and self.expand:
                ...
                input_size = units[i - 1]
            #z.size torch.Size([1024, 32])
            #w1.size torch.Size([32, 1024])
            # print("w1.size", self.w1[index].size())

...
######################################################################
LEARNING_RATE=0.001
CHECKPOINT_PATH=../checkpoint/WWW2023/${dataset}_${model}/duet
######################################################################
...
LOG_FILE="./logs/duet_${model}_$(date +%Y%m%d_%H%M%S).log"
nohup python ../main/multi_metric_meta_train.py \
--dataset=${data} \
${USER_DEFINED_ARGS} > ${LOG_FILE} 2>&1 &
